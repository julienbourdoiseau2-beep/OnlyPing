...
export function LegalPage({ title, updatedAt, children }: LegalPageProps) {
  return (
    <section className="mx-auto max-w-4xl px-3 sm:px-4 py-12">
      ...
      <p className="mt-2 text-sm text-ink-muted">Derniere mise a jour : {updatedAt}</p>
      <div className="legal-body mt-8 rounded-md border border-line bg-surface shadow-resting p-6 text-sm leading-relaxed text-ink-muted">
        {children}
      </div>
    </section>
  );
